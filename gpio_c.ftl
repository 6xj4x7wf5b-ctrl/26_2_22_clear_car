...
[#list datas as data]
    [#if data.ipName=="gpio"]
[#if data.comments?? && FamilyName!=("STM32N6")]
#n/** ${data.comments}
*/
[/#if]
...
[#if clock?size >0 ]#n#t/* GPIO Ports Clock Enable */[/#if]
        [#list clock as clockMacro]
            [#if clockMacro!=""]
			[#if ContextProject?? && (ContextProject?starts_with("A35TD") || ContextProject?starts_with("M33TD"))]
			[#else]
...
[/#if]
                    ...
                    [#compress]
[#if isLPGPIO == "false"]
                    #n#t/*Configure GPIO [#if pin?split("|")?size>1]pins[#else]pin[/#if] : [#list pin?split("|") as x][#if x?contains("GPIO_PIN")][#assign i =x?split("_")] [#assign j =i?last]${port}${j}[#else]${x} [/#if] [/#list] */
[#else]
...
[#list InitNvic as initVector]
     [#if initVector.label?? && initVector.label != "" && FamilyName==("STM32N6")]
